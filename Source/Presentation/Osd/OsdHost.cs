...
/// </summary>
internal static class OsdHost
{
    internal static OnScreenDisplayNotification Notification { get; set; } =
        new(OnScreenDisplayNotificationKind.Microphone, 0);
    ...
    private static IntPtr WindowProcedure(IntPtr window, uint message, UIntPtr wParam, IntPtr lParam)
    {
        switch (message)
        {
            case WmMicStatusChanged:
                Notification = new OnScreenDisplayNotification(
                    OnScreenDisplayNotificationKind.Microphone,
            ...
            case WmKeyboardBacklightChanged:
                ...
                {
                    Notification = new OnScreenDisplayNotification(
                        OnScreenDisplayNotificationKind.KeyboardBacklight,
                        keyboardLevel);
                    OsdPresenter.ShowStatusWindow(window);
                }
                return IntPtr.Zero;
            ...
            case WmDisplayBrightnessChanged:
                ...
                {
                    Notification = new OnScreenDisplayNotification(
                        OnScreenDisplayNotificationKind.DisplayBrightness,
                        brightness);
                    OsdPresenter.ShowStatusWindow(window);
                }
                return IntPtr.Zero;
            ...
            case WmPerformanceGpuChanged:
                ...
                {
                    Notification = new OnScreenDisplayNotification(
                        OnScreenDisplayNotificationKind.PerformanceGpuMode,
                        performanceGpuMode);
                    OsdPresenter.ShowStatusWindow(window);
                }
                return IntPtr.Zero;
            ...
            case WmTimer:
                ...
                if (wParam == HideTimerId)
                {
                    ...
                    OsdPresenter.HideStatusWindow(window);
                    return IntPtr.Zero;
                }
            ...
            case WmAnimationFrame:
                OsdPresenter.AdvanceWindowAnimation(window, unchecked((uint)wParam.ToUInt64()));
                return IntPtr.Zero;
            ...
            case WmPaint:
                OsdRenderer.PaintStatus(window);
                return IntPtr.Zero;
            ...
            case WmClose:
                KillTimer(window, HideTimerId);
                OsdPresenter.CancelShowAnimation(window);
                OsdPresenter.CancelHideAnimation(window);
    ...
    private static void PostNotification(IntPtr window, OnScreenDisplayNotification notification)
    {
    ...
    private static uint GetNotificationMessage(OnScreenDisplayNotificationKind kind)
    {
        return kind switch
        {
            OnScreenDisplayNotificationKind.Microphone => WmMicStatusChanged,
            OnScreenDisplayNotificationKind.KeyboardBacklight => WmKeyboardBacklightChanged,
            OnScreenDisplayNotificationKind.DisplayBrightness => WmDisplayBrightnessChanged,
            OnScreenDisplayNotificationKind.PerformanceGpuMode => WmPerformanceGpuChanged,
            _ => 0,
        };
    }
}
