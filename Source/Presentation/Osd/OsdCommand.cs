    ...
    public static bool TryHandle(string[] args, out int exitCode)
    {
        ...
        if (args.Length > 1)
        {
            ...
            else if (args[1].Equals(KeyboardBacklightCommandName, StringComparison.OrdinalIgnoreCase))
            {
                ...
                notification = new OnScreenDisplayNotification(OnScreenDisplayNotificationKind.KeyboardBacklight, level);
            }
            else if (args[1].Equals(DisplayBrightnessCommandName, StringComparison.OrdinalIgnoreCase))
            {
                ...
                notification = new OnScreenDisplayNotification(OnScreenDisplayNotificationKind.DisplayBrightness, brightness);
            }
            else if (args[1].Equals(PerformanceGpuCommandName, StringComparison.OrdinalIgnoreCase))
            {
                ...
                notification = new OnScreenDisplayNotification(OnScreenDisplayNotificationKind.PerformanceGpuMode, modeValue);
            }
            else
            {
                exitCode = 2;
